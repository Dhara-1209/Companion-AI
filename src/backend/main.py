...
# Configure logging with metrics FIRST before any logger usage
logging.basicConfig(
    level=logging.INFO,
...
class HomeBuddy:
    ...
    def process_query(self, query: str, chunks: list, brand: str = None, model_name: str = None):
        ...
        fallback_answer = "For troubleshooting assistance, please describe: (1) What the appliance is doing or not doing, (2) Any error codes displayed, (3) When the problem started. The system will retrieve relevant guides from the knowledge base."
        
        return {
            "answer": fallback_answer,
            "sources": [{"filename": chunk.get('source', 'knowledge_base'), "relevance_score": chunk.get('relevance_score', 0.5)} for chunk in chunks[:2]] if chunks else [],
            "confidence_score": 0.7 if chunks else 0.5,
            "safety_flag": any(word in query_lower for word in ["gas", "electrical", "shock", "fire", "burning"]),
            "safety_level": "caution" if any(word in query_lower for word in ["gas", "electrical", "shock", "smoke", "burning"]) else "safe",
            "safety_message": "⚠️ WARNING: This issue may require professional assistance for safety reasons." if any(word in query_lower for word in ["gas", "electrical", "shock", "smoke", "burning"]) else ""
        }
